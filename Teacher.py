...
def Evaluate(): #Evaluation GUI
	...
	def start(): #Begin evaluation 
		...
		def gen_key(exam_dir): #Generate the key
			...
			for file in os.listdir(os.getcwd(),exam_dir):
				if file[-7:-4] == 'Key':
					key_filename = os.path.join(os.getcwd(),exam_dir, file)

			for file in os.listdir(os.path.join(os.getcwd(),exam_dir, 'Raw Data')):
				if file[-11:-4] == 'Details':
					det_filename = os.path.join(os.getcwd(),exam_dir, 'Raw Data', file)

			with open(os.getcwd(),det_filename, 'r') as file:
				cont = eval(file.read())
				batch = len(cont[5])//3
		...
		def calculate(key, ans_filename): #Calcualte and produce the output
			...
			pdf.output(os.path.join(os.getcwd(),'Exam Results', parent_folder, f'{rpos}{student_det.split("_")[0]} Results {student_det.split("_")[2]} {student_det.split("_")[1]}.pdf'))
			rpos += 1
		...
		prog = 0
		try:
			os.mkdir(os.path.join(os.getcwd(),'Exam Results', parent_folder))
		except:
		...
		with open(os.path.join(os.getcwd(),'Exam Results', parent_folder, f"""_{exam} Emails_.txt"""), 'w+') as ifile:
...
def Results(): #Results GUI
	...
	def MassMail(): #Mass mail resuts to all the students
		...
		for f in list_dir[:-1]:
			file_list.append(os.path.join(os.getcwd(),'Exam Results', f'{tab} Results', f))
		with open(os.path.join(os.getcwd(),'Exam Results', f'{tab} Results', list_dir[-1]), 'r', encoding="utf8") as infile:
	...
	def Open(): #View result as a table
		...
		try:
			...
			open_res.iconbitmap(os.path.join(os.getcwd(),file_dir, 'logo.icns'))	
...
def otpValidation(): #Validate OTP
	...
	def otpRun(action, message = None): #Open the current channe in webbrowser
		global stop
		with open(os.path.join(os.getcwd(),'Program Files', 'Cache', "OTP Channel.txt"), 'r') as file:
			chlink = file.read()
			file.close()
		if chlink != '':
			try:
	...
	head_img = Image.open(os.path.join(os.getcwd(),file_dir, 'verification.png'))
	head_img = head_img.resize((193, 45), Image.ANTIALIAS)
	head_img = ImageTk.PhotoImage(head_img)
	head_lab = Label(top_frame, image = head_img, bg = 'white')
	head_lab.image = head_img
...
#__Main GUI__
def setup(window):
	window.iconbitmap(os.path.join(os.getcwd(),file_dir, 'logo.icns'))
...
def orgSet(): #Called form Main if mode is set to organisation 
	...
	root.configure(bg = 'white')
	root.title('Login')
	file_dir = os.path.join(os.getcwd(),'Program Files', 'Data Files')
	setup(root)
	top_frame = Frame(root, bg = 'white')
	top_frame.pack(side = 'top', fill = 'x', pady = (10,0))
	head_img = Image.open(os.path.join(os.getcwd(),file_dir, 'verification.png'))
